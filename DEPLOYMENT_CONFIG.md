...
| `LEMONSQUEEZY_WEBHOOK_SECRET` | 🔒 | On **Fly (api)** — webhook HMAC verification. **Required before payments go live** (else webhook degrades to accept-and-ignore, D045) |
| `LEMONSQUEEZY_API_KEY` | 🔒 | On **Vercel (web)** — used by `app/api/checkout/route.ts` hosted-checkout call |
| `LEMONSQUEEZY_STORE_ID` | 🔒 | On **Vercel (web)** |
| `LEMONSQUEEZY_VARIANT_MONTHLY` | 🔒 | On **Vercel (web)** — $19/mo variant ID |
| `LEMONSQUEEZY_VARIANT_ANNUAL` | 🔒 | On **Vercel (web)** — $179/yr variant ID |
...
- ✅ **Scheduled cadence (2026-06-16; cron moved to 06:00 UTC 2026-06-29, D108)** — `daily-brief.yml`
  cron enabled (`0 6 * * *`, 06:00 UTC; ingest once → publish all 3 desks). The early start absorbs
  GitHub's 40–100 min scheduler delay so a delayed run still lands before the morning reader window.
  Made safe to run unattended by D072 (regenerate on a bad draw/exception). Still subject to daily LLM spend.
- ✅ **Supabase URL config (2026-06-16)** — Site URL + Redirect allowlist set to
  `hard-power-intelligence.vercel.app`; verified with a test signup (email routes correctly).
- ✅ **`SUPABASE_SERVICE_ROLE_KEY` (2026-06-16)** — set on Fly + `.env`. Note: **not used by any
  service** (DB via `DATABASE_URL`, auth via `SUPABASE_JWT_SECRET`); reserved for future Supabase
  Admin API (e.g. comp-grants). The new Supabase `sb_secret_` API-key system is *not* adopted yet.
- ⬜ **Lemon Squeezy go-live** — checkout/webhook code is built (D050); needs account creds +
  variant IDs (see env matrix: API_KEY/STORE_ID/VARIANT_* on Vercel, WEBHOOK_SECRET on Fly).
- ⬜ **CI deploy workflow** (optional) — `meridian.yml` runs only structural verify. Note: the
  Depot remote builder can fail DNS here — deploy with `--local-only` (see DEPLOY_RUNBOOK §2.3).
