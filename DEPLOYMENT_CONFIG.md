...
- 🟡 **Production ingestion runner** — ✅ built (`engine/ingest/` + `scripts/run_ingest.py`,
  D057): live HTTP fetch + dedup + embed + cursor/accounting + circuit breaker + hot-window
  retention; USAspending wired live and `daily-brief.yml` ingests before briefing. ⬜
  Remaining: trigger the first production ingest run to replace the seeded fixtures with live
  data, then register more adapters (SEC EDGAR next, D055 §10).
...
- ⬜ **`persist_brief` idempotency** — replace an existing `(desk, date)` brief on rerun.
