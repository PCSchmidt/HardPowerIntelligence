...
- ✅ **Scheduled cadence (2026-06-16)** — `daily-brief.yml` cron enabled (`0 9 * * *`, 09:00 UTC;
  ingest once → publish all 3 desks). Made safe to run unattended by D072 (regenerate on a bad
  draw/exception). Still subject to daily LLM spend.
