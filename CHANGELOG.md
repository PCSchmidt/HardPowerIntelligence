...
- **Source publication dates on each item's sources — staleness is now visible** (2026-07-06, D129): the reader showed only our retrieval date (`fetched_at`, ~today for a daily run), so old and fresh sources looked identical. Each source's own publication/action date (already captured per adapter) is now normalized, persisted on the citation (`citations.published_at`), served by the API, and shown on the source card as "Published {date}" — falling back to "Retrieved {date}" when a source has no usable date, so nothing is ever dropped for lacking one. +18 tests. (Needs the migration applied + API/web redeploy to light up.)
- **Run observability completed — quality canaries, a daily digest, and cost tracking** (2026-07-05, D127): finishing Phase A on top of D126. The health check now also warns on a **collapsed confidence-mix** (a published brief with no confirmed/reported items) and a **leaked-JSON regression** (a D118 canary over item bodies/analysis). It emits a **daily digest** every run — per-desk item counts + attribution mix, per-source ingest volume, and the run's LLM token/cost total — so a green day still gives a one-glance picture. **Cost observability**: the LLM client tallies tokens per desk-run, `run_brief` stamps them on the brief, and the check warns above a per-run token budget. +14 tests.
- **Run-health alerting — the cron now pages on silent degradation, not just crashes** (2026-07-05, D126): the daily workflow's only alarm was GitHub's native "workflow failed" email, which fires only on a hard crash — so a failed ingest source, an open circuit breaker, a stale source, or a total publish shutout that exited cleanly never surfaced. A pure health evaluator (`engine/engine/ops/health.py`) + `scripts/run_health.py` + a scheduled `health` workflow job now inspect the run's own telemetry (briefs / ingestion_runs / source_registry) and go red (→ existing email channel) on those conditions, while correctly treating a normal thin-day skip as non-alerting. First gate of Phase A (`docs/PHASE_PLAN.md`); +16 tests. No new alerting infrastructure.
...
- **RSS feed registry expanded 20 → 40, scoped to the Defense∩AI∩Energy investment nexus** (2026-07-02/07-04):
  batch 1 — Defense News, C4ISRNET, Atlantic Council; The Next Platform, Import AI, SemiWiki, Brookings; Energy
  Storage News, Latitude Media, Heatmap News. Batch 2 (operator source lists) — TechCrunch AI, VentureBeat AI,
  MIT Tech Review AI, Hugging Face, OpenAI, The Gradient (AI); Power Engineering, ESS News, Carbon Tracker,
  Nuclear Newswire/ANS (energy). Coverage now 11/17/12; the AI weighting is deliberate — the primary-lab feeds
  (Hugging Face, OpenAI) fill the private-lab news gap the AI desk was missing. Feeds are the reliable,
  rate-limit-free news backbone (no GDELT IP problem). Each URL validates on the next CI run; a dead feed
  isolates itself (per-feed try/except). +1 dup-URL guard. Note: doubling the firehose raises froth/volume, so
  the deferred value-model work (materiality/significance for no-$ news) matters more as this grows.

### Fixed

- **An LLM-invented `item_type` no longer sinks a brief that already passed the gate** (2026-07-15, D140):
  on 7/15 the Defense brief passed on **attempt 1 with 40 provable claims** — D139 worked, the citation
  collapse was gone — and then died at the last step: `asyncpg.CheckViolationError: new row for relation
  "brief_items" violates check constraint "brief_items_item_type_check"`. Synthesis had labeled the CENTCOM
  one-way-attack-USV item `item_type: "operational"`. The CHECK allows only `award|filing|policy|macro|signal`,
  a taxonomy fixed on 2026-06-05 when the corpus was EDGAR-only (awards + filings). The net has since widened
  to news, arXiv and agency feeds, so the model now reaches for labels the 06-05 taxonomy never anticipated —
  and because persist runs in one transaction, **one unmapped word rolled back the entire brief**, leaving no
  row at all and taking the desk dark for a second day. The same class of bug as D139: an unvalidated
  free-text LLM field meeting a hard downstream contract. **Fix:** `normalize_item_type` coerces to the five
  allowed values — exact match, then a small nearest-fit synonym map (`contract`→award, `regulation`→policy,
  `operational`/`research`/`military`→signal), then per-component matching for compounds
  (`contract_award`→award) — and anything still unrecognized degrades to the catch-all `signal` rather than
  raising. Applied at the parse choke point **and** again at the INSERT, since that is where the constraint
  actually bites and no path should be able to violate it. The synthesis prompt now states the enum is
  exhaustive instead of merely listing it. **Deliberately not fixed by widening the CHECK:** the web's
  `ITEM_LABEL`/`ITEM_ICON`/`ITEM_BG`/`ITEM_TEXT` maps are `Record<ItemType, …>`, so an unknown type renders
  `undefined` — loosening the DB would only move the crash into the reader's browser. The constraint is
  load-bearing; the coercion belongs upstream of it. +18 tests (incl. the exact `"operational"` label and an
  invariant that no output can violate the CHECK); 629 backend green. _Open question deferred: the taxonomy
  is genuinely EDGAR-era, and mapping combat operations and arXiv research onto "signal" is lossy — widening
  it to `operational`/`research` is a real product call (migration + `ItemType` union + four web maps + chip
  colors), tracked separately. Resilience first: today the desk publishes._
- **A citation-format drift no longer silently darks a whole desk** (2026-07-14, D139): on 7/14 the Defense synthesis produced a complete, high-quality brief — valid JSON with a full headline, a 1,197-char BLUF, and convergence analysis — but **0 items survived**. Every LLM call succeeded (deepseek-v4-pro returned 10K+ completion tokens on each of 3 attempts; **not** credit exhaustion, **not** a provider outage), yet the D058 citation invariant deletes any sentence lacking the exact `[CITE:N]` token, and that day's draw wrote item-body citations in a near-miss format — so `strip_uncited_sentences` stripped every sentence, dropped all items → 0 provable claims → eval FAILED → `status=failed`. The three re-rolls reproduced it because a blind full re-synthesis retries the wrong thing. **Fix (parsing):** `normalize_citations` repairs the common drifts to `[CITE:N]` *before* enforcement — `[CITE 12]`, `[CITE: 12]`, `[cite-12]`, bare `[12]`, full-width `【CITE:4】` — without weakening the rule (a genuinely uncited sentence is still dropped; ranges like `[1-2]`, bare parentheticals `(2)`, and 4-digit years are left alone). **Fix (monitoring):** the health check now distinguishes a *citation collapse* (a failed brief with a headline but 0 items → **warn** `brief_items_collapsed`, which pages) from a genuine thin day (some items, just below the claim floor → INFO) — closing the blind spot where a dark desk hid behind "2/3 published". +21 tests. Backend-only; effective on the next run.
- **Type-aware "$X tracked" — a market projection no longer inflates the headline capital total** (2026-07-09, D138): the at-a-glance ledger's *"≈$X tracked"* figure and its magnitude bars were built by `keyAmount`, a pure `$`-regex over the item text with no notion of what a figure *means* — so a single **$720B market/forecast projection** (e.g. "$720B in U.S. grid investment needed by 2030") was summed into "tracked" and drawn as a full-width bar exactly like a real $28B acquisition, and it alone set the bar scale (flattening every genuine deal to a sliver). This inflated the Energy ($734.8B) and AI ($777.2B) headline totals on 7/9. The backend was already disciplined here — materiality only magnitude-weights structured `amount_usd` (award/filing amounts), never projection prose — so this is a **frontend-only** fix matching that discipline: a new `classifyAmount` tags each figure `tracked` (committed capital) vs `projected` (market-size/forecast language), classified from the same segment the figure was read from. Only *tracked* figures count toward the total and the magnitude scale; projections are **shown, not hidden** — rendered with a dashed "projected" tag instead of a proportional bar (per the honest-ledger principle, D084/D129). Applied in both `brief-glance` and the inline `brief-content` bars. Also fixed a latent gap where the single-letter `$2T` abbreviation parsed as `$2`. +8 web tests. Frontend-only; live immediately on redeploy (no run needed).
- **Active awards no longer read "Published 2022" — show the action date, not the contract-start date** (2026-07-09, D137): D129 chose USAspending's `start_date` (period-of-performance start) as the source's "Published" date, so a currently-live multi-year award surfaced as *"Published 9/30/2022"* — looking like stale content in a brief dated 7/9/2026. But the PoP start is a contract term, not a publication date; the award's own **Last Modified Date** (already ingested) is what put it in our window and is the honest recency cue. `published_at` now prefers `last_modified` and falls back to `start_date` only when the API omits it. The D123 4-year recency floor is deliberately left as-is (gating a live long-duration award on its PoP start would wrongly drop it — that's a significance call, not a date cutoff). +1 test. Backend-only; applies to briefs generated on the next run.
- **Syndicated near-duplicate collapse — one wire story no longer occupies dozens of slots** (2026-07-09, D135): the 7/9 Defense wire carried ~30 copies of "Trump Revives Calls For U.S. Control Of Greenland" (and ~8 of the Turkey/F-35 story) — GDELT syndicates one story across many local-radio sites, each a distinct `native_id` the ingest-level `(source_id, native_id, content_hash)` dedup can't see. `dedupe_by_title` now collapses candidates whose headline normalizes identically (case/punctuation-insensitive, trailing "| station-name" tail stripped), keeping the highest-scoring copy, applied before novelty/diversity/selection so it cleans both the brief pool and the wire. Conservative (exact normalized-title only; digits kept so "F-35"/"F-22" stay distinct) and it cuts significance-LLM tokens by not re-scoring the same headline N times. +12 tests. Backend-only; live on the next run.
- **RLS enabled on the 5 operational tables the initial schema missed** (2026-07-07, D134): the Supabase security advisor flagged `entity_identifiers`, `entity_aliases`, `source_registry`, `ingestion_runs`, `calendar_events` as `rls_disabled_in_public` (CRITICAL — readable/writable via the public anon key). Enabled RLS + REVOKE'd anon/authenticated grants, matching the `brief_wire` (D112) / lock-briefs (Gate 8) pattern. **No policy needed**: these are read only server-side by FastAPI/worker via the RLS-bypassing Postgres role, and the browser never reads them directly (no `supabase.from()` anywhere, D011) — so RLS-on-no-policy is deny-all to clients with zero API impact. Captured as a migration to fix repo↔prod drift after the fix was first applied by hand in the dashboard.
- **Transient source flakiness no longer fails an otherwise-healthy run** (2026-07-07, D132): the 7/7 run went red purely because SEC EDGAR 500'd and GDELT timed out — yet all three desks published fine. Under the old rule *any* health warning failed the run, which would perpetually reset the Phase-A "7 clean unattended days" gate on routine upstream hiccups. Source failures / open breakers / stale sources are now a non-failing **"notice"** tier (surfaced in the digest, exit 0); genuine degradation (shutout, thin/low-faithfulness brief, confidence collapse, JSON leak, cost anomaly) still fails. The safety net holds: if a source outage actually starves a desk below the item floor, the desk-level gate still escalates to a failure. +1 test (net), existing source-health tests updated.
- **Eval/significance calls get a fallback model — the deeper Defense fix** (2026-07-06, D130): D128 stopped the crash but Defense still failed — synthesis succeeded while the *eval* model (qwen3.7-max) returned `finish_reason='error'` under Defense's higher eval-call volume, and those calls had no fallback (`fallbacks=[]`). Added `llm_model_eval_fallback` (deepseek) and threaded it through the citation-eval and significance calls, so an eval-model provider error now degrades to the fallback instead of darkening the largest desk. +1 test.
- **Null provider content no longer crashes a desk** (2026-07-06, D128): the 7/6 Defense brief crashed all three attempts when the synthesis provider returned `finish_reason='error'` with null content — litellm raised no exception (so its fallbacks didn't fire) and the `None` hit `.strip()`. It struck Defense because it's the largest desk (33–35 candidate items → biggest prompt). `parse_json` is now null-safe and `complete()` explicitly tries the fallback model on empty content, then raises a clean error the re-roll handles. The A1 health check correctly caught the failure on its first live run. +7 tests.
- **Federal-award recency floor — decades-old ceilings no longer surface as "material today"** (2026-07-05, D123): a $22.4B Boeing/NASA award with a 1993 start date appeared in the 7/5 Defense wire because a recent administrative modification fell inside the 45-day lookback. The usaspending adapter now drops awards whose period of performance started before a 4-year horizon (fail-open on missing/unparseable dates), so legacy parent/IDIQ ceilings resurfacing on a mod are cut while normal multi-year awards stay. +4 tests.
